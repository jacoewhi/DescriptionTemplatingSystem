...
define("partName", "100-104IECContactor");
define("partPattern", "100-");
define("rulePattern", "/[A-Z]\d+\s--\s\d+-\d+|[A-Z]\s--\s\d+-\d+/");
define("specialRulePattern", "/\*/");
define("testProduct", "100-A18ND3");
...
if (isset($_POST['series'])) {
    if ($_POST['series'] == partName) {
        $query = $db->prepare("SELECT * FROM products WHERE part_number LIKE '10_%-K%'");
        $query->execute();
    ...
    foreach ($query as $result) {
        if ($_POST['series'] == partName) {
            if (!preg_match('/100-K/', $result['part_number']) &&
	            !preg_match('/104-K/', $result['part_number']) &&
	            !preg_match('/100S-K/', $result['part_number']) &&
	            !preg_match('/104S-K/', $result['part_number'])) {
                continue;
            }
        }
        $product = new Product();
...
<div class="container">
    <div class="row">
        <div class="col-lg-12" id="mainBody">
            <form class="col-lg-12" enctype="multipart/form-data" method="post" action="<?php echo partName ?>.php" style="text-align: center">
