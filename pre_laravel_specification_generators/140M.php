...
define("partName", "140M");
define("partPattern", "/140M-[CDF][28][ETV]-[A-E]\d{2}-(KN|KNY|TE|MT)-[XABCDERS][XCDEKLMNQ]-[GP]([JKDCHFATNBM]|VC)/");
define("rulePattern", "/[A-Z]\d+\s--\s\d+-\d+|[A-Z]\s--\s\d+-\d+/");
define("specialRulePattern", "/\*/");
define("testProduct", "140M-C2E-A16");
...
if (isset($_POST['series'])) {
    if ($_POST['series'] == partName) {
        $query = $db->prepare("SELECT * FROM products WHERE part_number LIKE '" . partName . "%'");
        $query->execute();
    ...
    foreach ($query as $result) {
        if ($_POST['series'] == partName) {
            if (!preg_match('/140M-/', $result['part_number'])) {
                continue;
            }
        }
        $product = new Product();
...
<style type="text/css">
    ...
    .tg td{background-color: #ffffff;border-color:#ccc;border-style:solid;border-width:1px;color:#333;
...
<div class="container">
    <div class="row">
        <div class="col-lg-12" id="mainBody">
            <form class="col-lg-12" enctype="multipart/form-data" method="post" action="140M.php" style="text-align: center">
                <h2 class="ui dividing header">140M Specification Generation</h2>
                ...
                <div class="row">
                    ...
                    <input type="radio" id="140M" name="series" value="140M">
                    <label for="140M">140M</label><br />
                </div>
