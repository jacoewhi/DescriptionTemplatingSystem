...
define("partName", "1326ABTorquePlus");
define("partDBPattern", "1326AB-%");
define("partSimplePattern", "(1326AB-B|1326AB-MOD)(4|5|7)(\d{2})");
define("partPattern", "(1326AB-B|1326AB-MOD)(4|5|7)(\d{2})([GJEHFC])-(21)?-?(S?M?)?2?([LK][457]?L?)?");
...
if (isset($_FILES['productCSV'])) {
	...
	foreach ($products as $product) {
		...
		foreach ($specs as $spec) {
			if (isset($product->specifications[$spec]) && $product->specifications[$spec] != "-") {
	...
	foreach ($products as $product) {
		...
		for ($i = 0; $i < count($keys) - 1; $i++) {
			if (isset($product->specifications[$keys[$i]]) && $product->specifications[$keys[$i]] != "-") {
		...
		for ($i = 0; $i < count($dimensions) - 1; $i++) {
			if (isset($product->specifications[$dimensions[$i]]) && $product->specifications[$dimensions[$i]] != "-") {
