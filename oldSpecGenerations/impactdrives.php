...
if (isset($_POST['series'])) {
    if ($_POST['series'] === "1336f") {
        $query = $db->prepare("SELECT * FROM products WHERE manufacturers_id = 45 AND Length(part_number) >= 16 AND part_number LIKE '1336F%'");
        $query->execute();
        $query = $query->fetchAll();
    } else if ($_POST['series'] === "1336s") {
        $query = $db->prepare("SELECT * FROM products WHERE manufacturers_id = 45 AND Length(part_number) >= 17 AND part_number LIKE '1336S%'");
        $query->execute();
        $query = $query->fetchAll();
    } else if ($_POST['series'] === "1336e") {
        $query = $db->prepare("SELECT * FROM products WHERE manufacturers_id = 45 AND Length(part_number) >= 16 AND part_number LIKE '1336E%'");
        $query->execute();
        $query = $query->fetchAll();
    } else if ($_POST['series'] === "1336t") {
        $query = $db->prepare("SELECT * FROM products WHERE manufacturers_id = 45 AND Length(part_number) = 16 AND part_number LIKE '1336T%'");
        $query->execute();
        $query = $query->fetchAll();
    }
    ...
    foreach ($query as $result) {
        if ($_POST['series'] === '1336f') {
            ...
            if (!(preg_match("/1336S-/", $result['part_number']))) {
                continue;
            }
        } else if ($_POST['series'] === '1336e') {
            if (!(preg_match("/1336E-[A-Z0-9]{4}-[A-Z]{2}-[A-Z0-9]{2}|1336F-[A-Z0-9]{5}-[A-Z]{2}-[A-Z0-9]{2}/", $result['part_number']))) {
                continue;
            }
        }
        $product = new Product();
...
<div class="container">
    <div class="row">
        <div class="col-lg-12" id="mainBody">
            <form class="col-lg-12" enctype="multipart/form-data" method="post" action="impactdrives.php" style="text-align: center">
...
<!--                    <label for="1326">13336T Series</label><br />                   -->
                </div>
